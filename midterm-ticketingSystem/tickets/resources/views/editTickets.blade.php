...
<body>
    ...
    <div class="main-content">
        <div class="container">
            ...
            @endif
            <form action="/update/{{$ticket[0]->id}}" method="post">
                @csrf
                <label for="title">Title:</label>
                <input type="text" name="title" value="{{$ticket[0]->title}}">
                @error('title')<span class="error">{{$message}}</span>@enderror
                
                <label for="description">Description:</label>
                <textarea id="description" name="description" rows="4">{{$ticket[0]->description}}</textarea>
